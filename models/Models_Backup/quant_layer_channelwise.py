...
def weight_quantization(b, grids, power=True):
    ...
    class _pq(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            i = (input.abs() > 1.).float()
            sign = input.sign()
            # grad_alpha = torch.sum(grad_output.clone(), (0, 2, 3), keepdim=True)  # (grad_output*(sign*i + (input_q-input)*(1-i))).sum()
            grad_alpha = (grad_output*(sign*i + (input_q-input)*(1-i))).mean(dim=(0, 2, 3), keepdim=True)
...
class weight_quantize_fn(nn.Module):
    def __init__(self, w_bit, in_channels, power=True):
        ...
        self.wgt_alpha = torch.nn.Parameter(1.0 * torch.ones(1, self.in_channels, 1, 1))
...
class QuantConv2d(nn.Conv2d):
    ...
    def show_params(self):
        wgt_alpha = torch.round(self.weight_quant.wgt_alpha.data, decimals=3)
        # print(wgt_alpha)
