...
def build_POT_diff(B=2):
    base_a = [0.]
    for i in range(B):
        base_a.append(2 ** i - 2 ** (-0.5 * i))
    for i in range(B):
        base_a.append(-1 * (2 ** i - 2 ** (-0.5 * i)))
    values = []
    for a in base_a:
        values.append((a))
    values = torch.Tensor(list(set(values)))
...
def weight_quantization(b, grids, power=True):
    ...
    class _pq(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            i = (input.abs() > 1.).float()
            sign = input.sign()
            y_error = input_q - input
            grad_input = grad_output.clone()*(1 + y_error)  # grad for weights will not be clipped
...
class weight_quantize_fn(nn.Module):
    def __init__(self, w_bit, power=True):
        ...
        # self.grids = build_POT_value(self.w_bit)
...
class F_alpha(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, alpha, weights, w_bit, grids):
        """
        ...
        """
        # ctx.save_for_backward(alpha, weights)
        # ctx.weights = weights
        weights.div_(alpha)  # weights are first divided by alpha
        weights_c = weights.clamp(min=-1, max=1)  # then clipped to [-1,1]
        sign = weights_c.sign()
        wt_abs = weights_c.abs()
        weight_q = power_quant(wt_abs, grids).mul(sign)  # Mapping the weight values to the POT steps
        ctx.save_for_backward(alpha, weight_q)
        forward_value = weight_q.mul(alpha)
        # Add the modifications to weight here after it is saved for backward pass
        # w_sign = torch.sign(weight_q)
        #     forward_value = w_sign*2**(torch.log2(torch.abs(w_in)))  # approximating the
        # forward_value = torch.where(w_in == 0, w_in, w_sign*2**(torch.log2(torch.abs(w_in))))
        # forward_value = torch.where(weight_q == 0, weight_q, alpha * w_sign * 2 ** (round_func_BPDA(torch.log2(torch.abs(weight_q)))))
        # forward_value = torch.where(weight_q == 0, weight_q, alpha * w_sign * 2 ** ((torch.log2(torch.abs(weight_q))).round()))
        # return alpha * (2 ** (weights) - 2 ** (-weights))
        return forward_value
    ...
    def backward(ctx, grad_output):
        ...
        """
        alpha, weights = ctx.saved_tensors
        # a = torch.tensor(2)
        #### Weighting the grad_weight by multiplying with error between y and y_q
        w_sign = torch.sign(weights)
        forward_value = torch.where(weights == 0, weights,
                                    alpha * w_sign * 2 ** ((torch.log2(torch.abs(weights))).round()))
        y_error = forward_value - weights
        # grad_alpha = (torch.tensor(2.0) ** (ctx.weights) - torch.tensor(2.0) ** (-ctx.weights)) * grad_output
        # grad_weights = alpha.clone() * torch.log(torch.tensor(2.0)) * (torch.tensor(2.0) ** (ctx.weights) + torch.tensor(2.0) ** (-ctx.weights)) * grad_output
        grad_alpha = weights.clone() * grad_output
        grad_weights = alpha.clone() * grad_output * (1 + y_error)
        return grad_alpha, grad_weights, None, None


class F_custom(torch.autograd.Function):
    """Both forward and backward are static methods."""
    ...
    def backward(ctx, grad_output):
        ...
        grad_weights = torch.log(torch.tensor(2.0)) * (
                torch.tensor(2.0) ** (ctx.weights) + torch.tensor(2.0) ** (-ctx.weights)) * grad_output
        return grad_weights


class QuantConv2d_Cust(nn.Module):  # Custom function
    def __init__(self, in_channel, out_channel, kernel_size=3, stride=1, padding=0, bias=True):
        super(QuantConv2d_Cust, self).__init__()
        self.k = kernel_size
...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        super(QuantConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation,
                                          groups,
                                          bias)
        ...
        self.wt_grids = build_power_value(self.bit, additive=True)
        # self.wt_grids = build_POT_value(self.bit)
        self.act_grid = build_power_value(self.bit, additive=True)
        self.act_alq = act_quantization(self.bit, self.act_grid, power=False)
    ...
    def forward(self, x):
        ...
        if self.bit == 32:
            ...
            weight = self.weight.add(-mean).div(std)      # weights normalization or zero center the weights
            weight_q = self.fn(self.wgt_alpha, weight, self.bit, self.wt_grids)

        # print(weight_q[0, 0, :, :])
        # print('weight alpha', self.wt_alpha)
