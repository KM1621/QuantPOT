...
class F_alpha(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_weights = alpha.clone()*grad_output
...
#@title rounding function POT
def pure_PoT(w_in):
    forward_value = torch.exp(w_in) - torch.exp(-w_in)
...
#         weight_feature = (2**(s_w) - 2**(-s_w))/s
        ...
        weight_feature = pure_PoT(self.weight.view(self.weight.size(0), -1).t())
...
#         print(stride)
        ...
        return out_    
class QuantConv2d_OLD(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        super(QuantConv2d_OLD, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups,
                                          bias)
