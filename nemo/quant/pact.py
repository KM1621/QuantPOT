...
criterion_qerr = nn.MSELoss().to(device)
...
class PACT_QuantFunc(torch.autograd.Function):
    ...
    def forward(ctx, input, eps, alpha, delta=0):
        ...
        input_q = ((input / (eps+delta)).floor() * eps).clamp(0., alpha.data[0]-eps.data[0])
        q_error = (input - input_q).abs()*where_input_nonclipped
        ctx.save_for_backward(where_input_nonclipped, where_input_gtalpha, q_error)
...
#        return ((input / (eps+delta)).floor() * eps).clamp(0., alpha.data[0]-eps.data[0])
    ...
    def backward(ctx, grad_output):
        ...
        where_input_nonclipped, where_input_gtalpha, q_error = ctx.saved_variables
        zero = torch.zeros(1).to(where_input_nonclipped.device)
        i = (where_input_nonclipped > 1.).float()
        grad_input = torch.where(where_input_nonclipped, grad_output * (i + q_error * (1 - i)), zero)
        grad_alpha = torch.where(where_input_gtalpha, grad_output * (i + q_error * (1 - i)), zero).sum().expand(1)
...
class PACT_QuantFunc_Signed(torch.autograd.Function):
    ...
    def forward(ctx, input, eps, alpha, delta=0):
        ...
        input_q = (input.clamp(-alpha.data[0], alpha.data[0]) / (eps+delta)).floor() * eps
        q_error = (input - input_q).abs()*where_input_nonclipped
        ctx.save_for_backward(where_input_nonclipped, where_input_gtalpha, q_error)
        #ctx.save_for_backward(where_input_nonclipped, where_input_gtalpha)
        return input_q
...
class PACT_QuantFunc_Asymm(torch.autograd.Function):
    ...
    def forward(ctx, input, eps, alpha, beta, delta=0):
        ...
        input_q = (input.clamp(-alpha_quant.item(), beta_quant.item()) / (eps+delta)).round() * eps
        q_error = (input - input_q).abs()*where_input_nonclipped
        ctx.save_for_backward(where_input_nonclipped, where_input_ltalpha, where_input_gtbeta, q_error)
    ...
    def backward(ctx, grad_output):
        # see Hubara et al., Section 2.3
        where_input_nonclipped, where_input_ltalpha, where_input_gtbeta, q_error = ctx.saved_variables
        zero = torch.zeros(1).to(where_input_nonclipped.device)
        i = (where_input_nonclipped > 1.).float()
        grad_input = torch.where(where_input_nonclipped, grad_output, zero)
        grad_alpha = torch.where(where_input_ltalpha, grad_output * q_error, zero).sum().expand(1)
        grad_beta  = torch.where(where_input_gtbeta, grad_output * q_error, zero).sum().expand(1)
...
class PACT_Conv2d(torch.nn.Conv2d):
    ...
    def forward(self, input):
        ...
        if self.training and self.quantize_x and not self.deployment:
            ...
            x_quant = input
            #print('activations not quantized')
        ...
        self.q_err_wt =   criterion_qerr(self.weight, W_quant) # q_err is scalar
        self.q_err_act =  criterion_qerr(input, x_quant)
