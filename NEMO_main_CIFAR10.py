...
parser.add_argument('--ptq', default=False, type=str, help='True for PTQ and False for full precision inference')
parser.add_argument('--pretrain', default=False, type=str, help='True for full precision training with builtin functions')
...
os.makedirs('./CifarOutput', exist_ok=True)
fdir = 'CIFAR_pretrained'
...
def main():
    ...
    if args.bit == 2 or args.pretrain:
        ...
        data = pd.read_csv('./CifarOutput/accuracy_tracesQAT' + str(args.qat) + '.csv')
    ...
    #optimizer = torch.optim.Adam(model.parameters(), lr=1e-2) #args.lr
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10('../data', train=True, download=False, transform=transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10('../data', train=False, download=False, transform=transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ...
    if args.pretrain:
        for epoch in range(args.epochs):
            ...
            }, is_best, args.bit, fdir)
        exit()
        

    if args.ptq:
        model = nemo.transform.quantize_pact(model, dummy_input=torch.randn((1,3,32,32)).to(device))
        precision = {
            'conv1': {
                'W_bits' : args.bit
            },
    ...
    if args.qat:
        ...
        plt.xlabel("epochs")
        fig.savefig('./CifarOutput/loss_' + str(args.bit) + '.png')
        fig = plt.figure()
        ...
        fig.savefig('./CifarOutput/q_error_' + str(args.bit) + '.png')
    ...
    #df.to_csv('./CifarOutput/accuracy_tracesQAT' + str(args.qat) + str(args.bit) + 'bit.csv')
    df.to_csv('./CifarOutput/accuracy_tracesQAT' + str(args.qat) + '.csv', index=False, header=True)
...
def save_checkpoint(state, is_best, bit, fdir):
    filepath = os.path.join(fdir, 'checkpoint' + str(bit) + '.pth')
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(fdir, 'model_best' + str(bit) + '.pth.tar'))
...
def adjust_learning_rate(optimizer, epoch):
    ...
    #adjust_list = [20, 60, 80]
    adjust_list = [5, 10, 15, 20, 60, 80, 120]
